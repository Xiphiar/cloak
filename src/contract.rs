...
//Snip 20 usage
use secret_toolkit::{
    snip20::handle::{register_receive_msg, transfer_msg},
...
pub fn handle<S: Storage, A: Api, Q: Querier>(
    ...
    match msg {
        ...
        HandleMsg::ChangeFee {
            new_fee,
            new_op_share,
        } => change_fee(deps, env, new_fee, new_op_share),
...
pub fn receive<S: Storage, A: Api, Q: Querier>(
    ...
    if let Some(bin_msg) = msg {
        match from_binary(&bin_msg)? {
            HandleReceiveMsg::ReceiveSeed {} => seed_wallet(deps, env, &mut config, gas_amount),
        }
    } else {
...
pub fn seed_wallet<S: Storage, A: Api, Q: Querier>(
    ...
    if !config.active {
        return Err(StdError::generic_err("Transfers are currently disabled"));
    }
    ...
    let new_data: String = format!(
        "{:?}+{}+{}+{}+{}",
        prng_seed, gas_amount, &env.block.height, &env.block.time, &env.message.sender
    );
    ...
    let cosmos_msg = redeem_msg.to_cosmos_msg(callback_code_hash, snip20_address, None)?;
    msg_list.push(cosmos_msg);
    ...
    Ok(HandleResponse {
        messages: msg_list,
        log: vec![log("tx_code", tx_key_string)],
        data: None,
    })
}
...
pub fn finalize_seed<S: Storage, A: Api, Q: Querier>(
    ...
    if env.message.sender != deps.api.human_address(&config.operator)? {
        return Err(StdError::generic_err("You are not a verified operator"));
    }
...
fn query_tx_exists<S: Storage, A: Api, Q: Querier>(
    deps: &Extern<S, A, Q>,
    tx_key: String,
